...
def solveSJFIO(self):
    ...
    while (completed != totalJobs):
        ...
        for i in range(totalJobs):
            if (ATlist[jobNumber[i]] == currTime and (not started[jobNumber[i]])):
                ReadyTemp.append(jobNumber[i])
                for j in range(len(JobTemp)):
                    if (JobTemp[j] == jobNumber[i]):
                        JobTemp.pop(j)
                        break

        # sorting the ready Queue according to shortest job
        ...
        # sorting the IO queue according to shortest job
            ...
            if (IOBTlist[IODeviceTemp[0]] == 0):
                ...
                if (BT2list[IODeviceTemp[0]] == 0):
                    ...
                    if (CStimeIO == self.ContextSwitchSelector.value()):
                        ...
                        completed += 1
                        TerminatedTemp.append(IODeviceTemp[0])
                        IODeviceTemp.pop(0)
                        if (CStimeIO != 0):
                            CStimeIO -= 1
                    ...
                    if (CStimeIO == self.ContextSwitchSelector.value()):
                        ...
                        ReadyTemp.append(IODeviceTemp[0])
                        IODeviceTemp.pop(0)
                        if (CStimeIO != 0):
                            IOCSdone = True
                            CStimeIO -= 1
                ...
                if (IOBTlist[CPUTemp[0]] == 0):
                    ...
                    if (BT2list[CPUTemp[0]] == 0):
                        ...
                        if (CStime == self.ContextSwitchSelector.value()):
                            ...
                            completed += 1
                            TerminatedTemp.append(CPUTemp[0])
                            CPUTemp.pop(0)
                            if (CStime != 0):
                                CStime -= 1
    ...
    while (self.IOCGTableJobs[len(self.IOCGTableJobs) - 1] == 'X' or self.IOCGTableJobs[
        len(self.IOCGTableJobs) - 1] == 'C'):
    ...
    for i in range(len(self.CGTableJobs)):
        ...
        if (self.CGTableJobs[i] != 'C' and self.CGTableJobs[i] != 'X'):
            item.setBackground(QtGui.QColor(
                self.rgbRed[int(self.CGTableJobs[i])], self.rgbGreen[int(self.CGTableJobs[i])],
                self.rgbBlue[int(self.CGTableJobs[i])]))
