...
def downloadBtnHandler(self):
    ...
    # creating the object
    workbook = xlsxwriter.Workbook("ProcessScheduler\\download.xlsx")
    ...
    worksheet.merge_range('G6:L6', str(
        self.ContextSwitchSelector.value()) + ' seconds')

    worksheet.merge_range('A7:F7', 'Time Quantum:', bold_format)
    worksheet.merge_range('G7:L7', str(
        self.TimeQuantumSelector.value()) + ' seconds')
    ...
    for i in range(self.JobNo.value()):
        worksheet.merge_range('A' + str(i+10) + ':C' + str(i+10), 'Job ' +
                              str(i+1), workbook.add_format({'bg_color': self.colorHexCode[i]}))
    ...
    # avg. TAT
    worksheet.merge_range('A' + str(self.JobNo.value() + 11) +
                          ':D' + str(self.JobNo.value() + 11), 'Avg. TAT:', bold_format)
    worksheet.merge_range('E' + str(self.JobNo.value() + 11) + ':K' +
                          str(self.JobNo.value() + 11), str(self.AvgTAT.text()) + ' seconds')

    # avg. WT
    worksheet.merge_range('A' + str(self.JobNo.value() + 12) +
                          ':D' + str(self.JobNo.value() + 12), 'Avg. WT:', bold_format)
    worksheet.merge_range('E' + str(self.JobNo.value() + 12) + ':K' +
                          str(self.JobNo.value() + 12), str(self.AvgWT.text()) + ' seconds')

    # avg. RT
    worksheet.merge_range('A' + str(self.JobNo.value() + 13) +
                          ':D' + str(self.JobNo.value() + 13), 'Avg. RT:', bold_format)
    worksheet.merge_range('E' + str(self.JobNo.value() + 13) + ':K' +
                          str(self.JobNo.value() + 13), str(self.AvgRT.text()) + ' seconds')

    worksheet.merge_range('A' + str(self.JobNo.value() + 15) + ':L' +
                          str(self.JobNo.value() + 15), 'Gantt Chart (CPU)', bold_format)
    worksheet.merge_range('A' + str(self.JobNo.value() + 17) +
                          ':B' + str(self.JobNo.value() + 17), 'Job', bold_format)
    worksheet.merge_range('A' + str(self.JobNo.value() + 18) +
                          ':B' + str(self.JobNo.value() + 18), 'Time', bold_format)
            ...
            if(j > 31):
                ...
                if(i == 0):
                    ...
                    if(value != 'C' and value != 'X'):
                        worksheet.write(rowid, colid, value, workbook.add_format(
                            {'bg_color': self.colorHexCode[int(value)-1]}))
                    else:
                        worksheet.write(rowid, colid, value)
                else:
                    if(checker[j-1] != 'C' and checker[j-1] != 'X'):
                        if(j > 99):
                            worksheet.write(rowid + 1, colid, value, workbook.add_format(
                                {'bg_color': self.colorHexCode[int(checker[j-1])-1], 'font_size': 7}))
                        else:
                            worksheet.write(rowid + 1, colid, value, workbook.add_format(
                                {'bg_color': self.colorHexCode[int(checker[j-1])-1]}))
                    else:
                        if(j > 99):
                            worksheet.write(
                                rowid + 1, colid, value, workbook.add_format({'font_size': 7}))
                        else:
                ...
                if(i == 0):
                    checker.append(value)
                    if(value != 'C' and value != 'X'):
                        worksheet.write(self.JobNo.value(
                        ) + 16, j + 1, value, workbook.add_format({'bg_color': self.colorHexCode[int(value)-1]}))
                    else:
                        worksheet.write(self.JobNo.value() + 16, j + 1, value)
                else:
                    if(checker[j-1] != 'C' and checker[j-1] != 'X'):
                        worksheet.write(self.JobNo.value() + 17, j + 1, value, workbook.add_format(
                            {'bg_color': self.colorHexCode[int(checker[j-1])-1]}))
                    else:
    ...
    if(self.TypeWithIOBT.isChecked()):
        lastrow += 2
        worksheet.merge_range('A' + str(lastrow + 2) + ':L' +
                              str(lastrow + 2), 'Gantt Chart (IO)', bold_format)
        worksheet.merge_range('A' + str(lastrow + 4) +
                              ':B' + str(lastrow + 4), 'Job', bold_format)
        worksheet.merge_range('A' + str(lastrow + 5) +
                              ':B' + str(lastrow + 5), 'Time', bold_format)
        checker = []
        changed = False
        if(self.GCchangeBtn.text() == 'IO Gantt Chart'):
            self.GCchangeBtnHandler()
            ...
            for j in range(1, len(self.IOCGTableJobs) + 1):
                ...
                if(j > 31):
                    ...
                    if(i == 0):
                        checker.append(value)
                        if(value != 'C' and value != 'X'):
                            worksheet.write(lastrow + 3, j + 1, value, workbook.add_format(
                                {'bg_color': self.colorHexCode[int(value)-1]}))
                        else:
                            worksheet.write(lastrow + 3, j + 1, value)
                    else:
                        if(checker[j-1] != 'C' and checker[j-1] != 'X'):
                            worksheet.write(lastrow + 4, j + 1, value, workbook.add_format(
                                {'bg_color': self.colorHexCode[int(checker[j-1])-1]}))
                        else:
    ...
    nameoffile = str(self.AlgoSelector.currentText()) + "_" + str(current_time.day) + "_" + str(current_time.month) + "_" + \
        str(current_time.year) + "__" + str(current_time.hour) + "_" + \
        str(current_time.minute) + "_" + str(current_time.second)
    ...
    books = xlApp.Workbooks.Open(
        str(abspath)+'\\download.xlsx')
    ws = books.Worksheets[0]
    ws.Visible = 1
    ws.ExportAsFixedFormat(
        0, str(abspath)+'\\'+str(nameoffile)+'.pdf')
    books.Close()
